		...
		try {
			...
			for (int accountNumber : user.getAccounts()) {
				if (accounts == null || accounts.get(accountNumber) == null) { // update only when it's not available
					...
					// int id, Status status, int accountNumber, int currUserId, int pin, AccountMessageType type
					AccountMessage msg = new AccountMessage(++id, Status.ONGOING, accountNumber, user.getId(), -1, AccountMessageType.ACCOUNT_INFO);
					// send message
